...
from scipy import stats  # 新增：用于 P‑value 假设检验
...
def main():  # noqa: D401
    ...
    # ===== P‑value 假设检验 =====
    # 1) 对皮尔逊相关性的显著性检验（H0: ρ = 0）
    _, p_corr = stats.pearsonr(preds, gts)

    # 2) 配对 t 检验：预测值与真实值均值差异（H0: μ_diff = 0）
    t_stat, p_pair = stats.ttest_rel(preds, gts)

    print(
        "\n[TEST] MAE {:.3f} | RMSE {:.3f} | R² {:.3f} | PCC {:.3f} (p={:.3e}) | Δμ t={:.3f} (p={:.3e})".format(
            mae, rmse, r2, pcc, p_corr, t_stat, p_pair
        )
    )

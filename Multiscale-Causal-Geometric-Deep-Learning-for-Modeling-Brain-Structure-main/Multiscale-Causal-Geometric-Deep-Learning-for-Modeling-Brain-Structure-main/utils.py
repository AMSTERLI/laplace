...
def load_cosgraph_json(json_path):
    ...
    for node in graph.nodes(data=True):
        # 使用归一化值作为节点特征
        node_features.append([node[1]['normalized_value']])
    ...
    return Data(
        ...
        y=torch.LongTensor([0])  # 根据实际标签修改
    )

# ===== Differentiable MI override (torch) =====
from utils_torch import (
    MI as _MI,
    calculate_sigma as _sigma,
    calculate_conditional_MI as _cmi,
)

# Re‑export to keep legacy imports working
MI = calculate_MI = _MI
calculate_sigma = _sigma
calculate_conditional_MI = _cmi
